...
def get_segment_path(directories: Dict[str, str], segment_number: str, profile_name: str = None) -> str:
...
def transcode_segment(directories: Dict[str, str], segment_number: str, 
                     input_filename: str, profile: Dict) -> None:
    ...
    try:
        ...
        logger.info(f"Successfully created {profile['name']} version of segment {segment_number}")
    except subprocess.CalledProcessError:
        logger.exception(f"FFmpeg transcoding failed for segment {segment_number}, profile {profile['name']}")

def process_segment(directories: Dict[str, str], segment_number: str, 
...
def process_messages(consumer: KafkaConsumer, process_fn: Callable, 
                    executor: ThreadPoolExecutor, directories: Dict[str, str]) -> None:
